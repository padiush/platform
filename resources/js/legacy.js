...
function pushSection(
    ...
    sectionDetailsCard.classList.add(
        'card',
        'w-full',
        'bg-base-200',
        'shadow-xl',
...
function pushTextInput(
    ...
    isMandatoryLabel.innerHTML =
        '<span class="label-text">¿Es obligatorio?</span>';
    ...
    if (create) {
        $.ajax({
            ...
            success: function (response) {
                ...
                isMandatoryCheckbox.setAttribute(
                    'name',
...
function pushSingleSelect(
    ...
    optionsLabel.innerHTML =
        '<span class="label-text">Opciones <span class="text-red-500">*</span></span>';
...
function createNewRepeatingSectionInstance(section_id, route, token) {
    ...
    instance.id =
        ...
        section_id +
        '-instance-' +
        (instancesContainer.childElementCount + 1);
    ...
    if (answerRepeatable.length > 0) {
        Object.values(answerRepeatable).forEach(function (element) {
            ...
            element.addEventListener('change', function () {
                ...
                var item_id = id
                    .replace('answer-repeatable-', '')
                    .replace(/-[^-]*$/, '');
                var repeatable_index = id.replace(
                    'answer-repeatable-' + item_id + '-',
                    '',
                );
...
function populateRepeatableSection(
    container,
    route,
    token,
    section_id,
